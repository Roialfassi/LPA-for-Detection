...
# Resolve test fixtures relative to this file so the suite is runnable from any
# working directory (e.g. `pytest` from the repo root).
...
@pytest.fixture
def freq():
    return pd.read_csv(DATA_DIR / "test_freq.csv")


@pytest.fixture
def real_dvr():
    return pd.read_csv(DATA_DIR / "test_dvr.csv")


@pytest.fixture
def real_pvr():
    return pd.read_csv(DATA_DIR / "test_pvr.csv")
